...
function exchange_build_social_icons( $context = '', $platforms = array(), $exchange = null ) {
	...
	$circle = '';
	$circle_fallback = '';
	if ( 'footer' === $context || 'section' === $context ) {
	...
	foreach ( $platforms as $button ) {
		$output .= '<li class="social-icons-list__item"><a class="social-icons-list__link link--' . $button;
		if ( $share_buttons ) {
			$output .= '" title="' . sprintf( $link_titles[ $button ], $exchange->type );
		} else {
			$output .= '" title="' . $link_titles[ $button ];
		}
		$output .= '" target="_blank" href="' . $button_links[ $button ] . '">'
