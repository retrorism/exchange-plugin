...
class Exchange {
	...
	public function publish_sharing_buttons( $context = '' ) {
		...
		$encoded_link = urlencode( $this->link );
		$icon_colour = '#' . $GLOBALS['EXCHANGE_PLUGIN_CONFIG']['COLOURS']['yellow-tandem'];
		$circle = '';
		$circle_fallback = '';
		if ( 'footer' === $context ) {
			...
			$icon_colour = '#' . $GLOBALS['EXCHANGE_PLUGIN_CONFIG']['COLOURS']['white'];
		}
		...
		$email_share_subject = $email_share_subject_option
			? rawurlencode( sprintf( $email_share_subject_option, $this->type, $this->title ) )
			: rawurlencode( sprintf( '%s from %s: %s', ucfirst( $this->type ), $site_name, $this->title ) );
		$email_share_text = $email_share_text_option
			? rawurlencode( sprintf( $email_share_text_option, ucfirst( $this->type ), $this->title, $this->link ) )
			: rawurlencode( sprintf( "Hi!\n\nThis %s on %s might interest you: %s\n\n", $this->type, $site_name, $this->title ) );
		...
		$button_links = array(
			'twitter'  => 'https://twitter.com/intent/tweet?text=' . urlencode( sprintf( __( '%s from %s: %s', EXCHANGE_PLUGIN ), ucfirst( $this->type ), $site_name, $this->title ) ) . '&url=' . $encoded_link,
			'facebook' => $facebook_link,
			'email'    => 'mailto:?subject=' . $email_share_subject . '&body=' . $email_share_text . '%0D%0A%0D%0A' . $encoded_link,
			'print'    => $this->link . '?exchange_display=print',
		);
		...
		foreach ( array_keys( $button_links ) as $button ) {
			$output .= '<li class="social-icons-list__item"><a class="social-icons-list__link link--' . $button
					. '" title="' . sprintf( $link_titles[ $button ], $this->type )
					. '" target="_blank" href="' . $button_links[ $button ] . '">'
					. $button_svgs[ $button ] . '</a></li>';
		}
		$output .= '</ul>';
		echo $output;
	}
